...
function runVictoryLoop() {
  ...
  // Только рисуем если canvas боя ещё активен
...
function endBattle(winner) {
  ...
  setTimeout(()=>{
    ...
    let icon,title,sub;
    if (timeout) {
      if (dw==='draw') { icon='⚖'; title='AEQUITAS';   sub='Tempus fugit — vires aequales sunt.'; }
      else if (win)    { icon='🏆'; title='VICTORIA';   sub='Tempus fugit — plures copiae tuae!'; }
      else             { icon='💀'; title='CALAMITAS';  sub='Tempus fugit — hostis superior erat.'; }
    } else {
      icon=win?'🏆':'💀';
      title=win?'VICTORIA':'CALAMITAS';
    ...
    document.getElementById('result-stats').innerHTML=`
      ...
      <div class="stat-row"><span>Потери</span><span>${G.kills.bot}</span></div>
      <div class="stat-row"><span>Выжило у игрока</span><span>${pa}</span></div>
      <div class="stat-row"><span>Выжило у бота</span><span>${ba}</span></div>
      <div class="stat-row"><span>Фракция игрока</span>
